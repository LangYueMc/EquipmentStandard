...
public class Attribute {
    ...
    public record Final(
        ...
        public Final(String type, boolean merge, double amount, Operation operation, Set<Slot> slots) {
            ...
            this.merge = merge;
            this.amount = amount;
            if (type.equalsIgnoreCase(CustomAttributes.CRIT_CHANCE.getDescriptionId())
                    || type.equalsIgnoreCase(CustomAttributes.CRIT_DAMAGE.getDescriptionId()))
                // 暴击和暴击伤害都是累加的，而且都是百分比，为了显示正常，这里固定为 MULTIPLY_BASE
                this.operation = Operation.MULTIPLY_BASE;
            else if (type.equalsIgnoreCase(CustomAttributes.REAL_DAMAGE.getDescriptionId()))
                // 真实伤害固定为 ADDITION
